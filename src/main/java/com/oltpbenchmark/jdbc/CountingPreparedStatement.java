...
/**
 * A PreparedStatement wrapper that counts SQL operations (executeQuery, executeUpdate, executeBatch).
 * Uses a shared AtomicLong counter provided at construction time.
 */
public class CountingPreparedStatement implements PreparedStatement {

    private final PreparedStatement stmt;
    private final AtomicLong operationsCounter;

    public CountingPreparedStatement(PreparedStatement stmt, AtomicLong operationsCounter) {
        this.stmt = stmt;
        this.operationsCounter = operationsCounter;
    }

    // Execute methods that count operations
    ...
    public int[] executeBatch() throws SQLException {
        int[] results = this.stmt.executeBatch();
        // Count each batch item as one operation
        operationsCounter.addAndGet(results.length);
        return results;
    }
    ...
    public long[] executeLargeBatch() throws SQLException {
        long[] results = this.stmt.executeLargeBatch();
        operationsCounter.addAndGet(results.length);
        return results;
    }

    // Delegate all other methods to wrapped statement
